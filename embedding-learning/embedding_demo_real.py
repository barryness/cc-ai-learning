'''
=============================================================================
  Embedding 实战 Demo（真实模型版） —— 从零理解语义搜索
  ───────────────────────────────────────────────────────────────────────
  运行方式：python3 embedding_demo_real.py
  ...
  与 embedding_demo.py 的区别：
  - embedding_demo.py：用 numpy 模拟向量，无需 PyTorch，可直接运行
  - embedding_demo_real.py：使用真实的 sentence-transformers 模型，
    向量来自海量文本训练，语义关系更准确。需要 PyTorch 环境。
=============================================================================
'''
import numpy as np
...
#  训练方式：在海量文本上，用「预测上下文词」的方式训练，
#  让「意思相近的文本」在高维空间中向量方向接近。
#  模型首次运行自动下载到本地缓存，之后走缓存。
# ═══════════════════════════════════════════════════════════════════════
...
def embed(texts):
    ...
    这行代码的背后：
        model.encode(texts) 把每个文本输入一个 6 层 Transformer 网络，
        经过 self-attention 计算，输出一个 384 维的「语义向量」。
        整个过程端到端训练——意思接近的文本，向量方向自然接近。
    """
    model = get_model()
...
# ═══════════════════════════════════════════════════════════════════════
#  Demo 1: 文本 Embedding —— 用真实模型把文字变成向量
# ═══════════════════════════════════════════════════════════════════════
...
def demo_embedding():
    ...
    print('  不是模拟数据——模型确实在海量文本中"学到"了汽车和轿车意思接近。')
...
def demo_real_similarity():
    ...
    # 真实模型不需要 category_map——模型自己知道哪些词意思接近
    vectors = embed(words)
...
def main():
    print()
    print('╔══════════════════════════════════════════════════════════════╗')
    print('║    Embedding 实战 Demo（真实模型版）                         ║')
    print('║    使用 all-MiniLM-L6-v2 真实语义模型                       ║')
    print('╚══════════════════════════════════════════════════════════════╝')
    print()
    print('Demo 路线图：')
    ...
    print()
    print('对比 embedding_demo.py（numpy 模拟版），二者的搜索、相似度、')
    print('可视化逻辑完全相同，只有 embed() 函数不同。')
    print('这就是"面向接口编程"——换一个模型实现，上层代码不变。')
    print()
