...
_INSTALL_HINT = '`pip install langchain-openai langchain-core python-dotenv tiktoken plotly`'


def _load_tiktoken():
    """按需导入 tiktoken，失败返回 None 和错误信息。"""
    try:
...
def _load_plotly():
    """按需导入 plotly，失败返回 None 和错误信息。"""
    try:
        import plotly.graph_objects as _mod
        return _mod, None
    except Exception as e:
        return None, f'{e}'
...
def _load_llm_libs():
    """按需导入 langchain，失败返回 None 和错误信息。"""
    try:
        ...
        return (_Chat, _HM, _SM), None
    except Exception as e:
        return None, f'{e}'


def _load_dotenv():
    """按需导入 dotenv，失败返回 None 和错误信息。"""
    try:
...
def _show_missing_dep(name, err):
    """显示友好的依赖缺失提示。"""
    st.error(f'### ⚠️ 缺少 `{name}` 模块\n\n**错误原因：** `{err}`\n\n**请运行：** {_INSTALL_HINT}')


# ─── .env 加载 ──────────────────────────────────

_load_dotenv_fn, _dotenv_err = _load_dotenv()
if _load_dotenv_fn is not None:
    _env_path = Path(__file__).parent / '.env'
    if _env_path.exists():
        _load_dotenv_fn(_env_path)
    else:
        _load_dotenv_fn()

# ─── 页面初始化 ────────────────────────────────

st.set_page_config(page_title='LLM 入门实验台', page_icon='🤖', layout='wide')

st.title('🤖 LLM 入门实验台')
st.markdown('> 不用写一行代码，点点按钮就能理解 LLM 的核心概念')

tab = st.sidebar.radio('选择实验', [
    '🏠 首页',
    '🔤 实验1: Token 分切机',
    '👁️ 实验2: Attention 可视化',
...
_INSTALL_HINT = '`pip install langchain-openai langchain-core python-dotenv tiktoken plotly`'
_PKG_MAP = {'tiktoken': 'tiktoken', 'plotly': 'plotly', 'llm': 'langchain-openai langchain-core', 'dotenv': 'python-dotenv'}


@st.cache_resource
def get_llm():
    """每次按钮点击时才导入 langchain（懒加载）。"""
    libs, err = _load_llm_libs()
    if libs is None:
        raise RuntimeError(f'缺少 LLM 依赖：{err}')
    ChatOpenAI, _, _ = libs
    return ChatOpenAI(
...
def _get_msg_classes():
    """懒加载 HumanMessage / SystemMessage，失败返回 (None, err)。"""
    libs, err = _load_llm_libs()
    if libs is None:
        return None, None, err
    _, HumanMessage, SystemMessage = libs
    return HumanMessage, SystemMessage, None
...
# ════════════════════════════════════════════════
    ...
    example = st.selectbox('选择一个示例', [
        '', 'Hello, world!', '中国的首都是北京',
        'Transformer是一种神经网络架构',
        '🌟 Hello 你好 123 !@#',
    ])
    user_text = st.text_area(
        '或者自己输入', value=example, height=100,
        help='输入任何文本，看它被拆成多少 Token',
    )

    if st.button('🔪 切分 Token', type='primary'):
        _tiktoken_mod, err = _load_tiktoken()
        if _tiktoken_mod is None:
            _show_missing_dep('tiktoken', err)
        else:
            enc = _tiktoken_mod.get_encoding('cl100k_base')
            ...
            if not text:
                ...
                for i, t in enumerate(tokens):
                    ...
                    safe_token = html.escape(decoded[i])
                    html_parts += (
                        f'<span style="background:{color}; padding:2px 6px; '
                        f'margin:2px; border-radius:4px; font-size:18px" '
                        f'title="ID:{t}">{safe_token}</span>'
                    )
                st.markdown(
                    f'<div style="line-height:2.5">{html_parts}</div>',
                    unsafe_allow_html=True,
                )

                with st.expander('查看 Token ID 列表'):
                    for i, (t, d) in enumerate(zip(tokens, decoded)):
                        st.text(f'  Token #{i:3d}: ID {t:6d} -> {d!r}')
...
# ════════════════════════════════════════════════
    ...
    with st.expander('📖 先理解 Attention（点击展开）'):
        st.markdown('''
        - 读「那家店在王府井」，看到「那家」时，大脑自动看向「店」「王府井」
        - Attention 做的就是这事：**每个词都算一个「关注度分数」**
        - 分数越高 → 这个词对当前词「越重要」
        - 最后每个词都吸收了其他词的信息，变成融合上下文的新表示
        ''')

    sentence = st.text_input('输入一句话（英文，用空格分隔）', value='The cat sat on the mat')
    words = sentence.strip().split()

    if len(words) < 2:
        st.warning('至少输入 2 个词')
    else:
        _go_mod, err = _load_plotly()
        if _go_mod is None:
            _show_missing_dep('plotly', err)
            st.stop()
        ...
        fig = _go_mod.Figure(data=_go_mod.Heatmap(
            z=weights, x=words, y=words,
            colorscale='Viridis',
...
# ════════════════════════════════════════════════
    ...
    fig.add_hline(
        y=1/n_tokens, line_dash='dash', line_color='gray',
    ...
    positions = {
        '开头': 0, '1/4处': doc_len // 4, '中间': doc_len // 2,
    ...
    fig2 = _go_mod.Figure(data=[_go_mod.Bar(
        x=labels, y=values,
...
# ════════════════════════════════════════════════
    ...
    **Prompt = 你给 LLM 的「起点」。起点不同，续写方向完全不同。**
    ...
    question = st.text_input('✏️ 你想问什么？', value='解释什么是机器学习')

    col1, col2 = st.columns(2)
    with col1:
        role = st.selectbox('🎭 让 LLM 扮演什么角色？', [
            '普通助手（无角色）',
            '资深数据仓库工程师',
            '幼儿园老师（给 5 岁小孩讲）',
            '毒舌程序员',
    ...
    role_prompts = {
        '普通助手（无角色）': '',
        '资深数据仓库工程师': (
            '你是一个资深数据仓库工程师。用数据库和数据仓库的类比来解释技术概念。'
            '回答要专业、有 SQL 示例。'
        ),
        '幼儿园老师（给 5 岁小孩讲）': (
            '你是一个幼儿园老师。用最简单的语言、比喻和故事来解释。'
            '每句话不超过 20 个字。用小朋友能懂的方式说。'
        ),
        '毒舌程序员': (
            '你是一个毒舌但技术很强的程序员。回答尖锐、幽默、一针见血。'
            '可以吐槽，但要说到点子上。'
        ),
        '古代诗人': (
            '你是一个中国古代诗人。用诗意的语言回答问题，适当引用诗句，语言典雅。'
        ),
        '产品经理': (
            '你是一个资深产品经理。从用户需求、场景、价值的角度回答问题。结构化思考。'
        ),
    }

    if st.button('🚀 发送', type='primary'):
        HumanMessage, SystemMessage, err = _get_msg_classes()
        if HumanMessage is None:
            _show_missing_dep('llm', err)
            ...
            with st.spinner('🤔 LLM 正在思考...'):
                ...
                fmt_text = {
                    '自由格式': '',
                    'JSON 格式': '\n请用 JSON 格式输出。',
                    'Markdown 格式': '\n请用 Markdown 格式输出，包含标题和列表。',
                    '表格格式': '\n请用表格格式呈现主要信息。',
                    '一句话概括': '\n请用一句话概括。不要超过 50 个字。',
                }.get(output_format, '')

                full_system = (role_text + fmt_text).strip()
                if full_system:
    ...
    demo_q = st.text_input('对比实验的问题', value='什么是机器学习')
    if st.button('🔄 开始对比', type='secondary'):
        ...
        if HumanMessage is None:
            ...
            roles_to_compare = [
                ('普通助手', ''),
                ('幼儿园老师', role_prompts.get('幼儿园老师（给 5 岁小孩讲）', '')),
                ('数据仓库工程师', role_prompts.get('资深数据仓库工程师', '')),
                ('毒舌程序员', role_prompts.get('毒舌程序员', '')),
            ]
            with st.spinner('🤔 调用 4 个角色对比...'):
                role_results = {}
                for role_name, role_prompt in roles_to_compare:
                    msgs = []
                    if role_prompt:
                        msgs.append(SystemMessage(content=role_prompt))
                    msgs.append(HumanMessage(content=demo_q))
                    role_results[role_name] = call_llm(msgs)

            # 用 expander 显示每个角色的结果（比 tabs 更稳定）
...
# ════════════════════════════════════════════════
    ...
    **幻觉 = LLM 编造看起来合理但实际错误的内容。**
    ...
    test_type = st.radio('选择测试类型', [
        '🔥 知识边界测试（问未来的事）',
        '✅ 基础知识测试（问 LLM 应该知道的事）',
        '✏️ 自定义测试（自己输入问题）',
    ])

    if '知识边界' in test_type:
        questions = [
            '2025 年诺贝尔文学奖得主是谁？',
            '2026 年世界杯决赛的比分是多少？',
                ...
                if HumanMessage is None:
                    ...
                    with st.spinner('🤔 思考中...'):
                        ...
                        has_hallucination = any(
                            kw in result for kw in ['抱歉', '不知道', '尚未', '无法']
                        )
        ...
        for q in questions:
            if st.button(f'❓ {q}', use_container_width=True):
                HumanMessage, _, err = _get_msg_classes()
                if HumanMessage is None:
                    _show_missing_dep('llm', err)
                else:
                    with st.spinner('🤔 思考中...'):
        ...
        if st.button('🔍 检测幻觉', type='primary'):
            ...
            if HumanMessage is None:
                ...
                with st.spinner('🤔 思考中...'):
                    ...
                    st.markdown(result)
                    st.caption(
                        '💡 **判断方法**：如果回答中有具体名称、数字、引用'
                        '但你不确定是否真实，可能是幻觉。可以用搜索引擎验证。'
                    )
